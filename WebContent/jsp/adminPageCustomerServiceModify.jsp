...
<!-- title 아이콘 -->
		...
		function check(){
			...
			alert("등록합니다.");
			document.frm.action="<%=request.getContextPath() %>/Admin2/adminPageCustomerServiceModifyAction.do";
	...
	<section>
		<article>
			<form class="pagefrmp" name="frm">
				<h2>공지사항 수정</h2>
				<input type="hidden" value="<%=cv.getCcategory()%>">
				<select name="ccategory" id="ncategory" style="width:120px;">
				...
				<table style="border-collapse:collapse">
					<tr>
						<td width=10%>제목</td>
						<td width=150%><input name="csubject" value="<%=cv.getCsubject()%>"></td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea name="ccontent"><%=cv.getCcontent()%></textarea></td>
					</tr>
					<tr>
						<td>작성자</td>
						<td><input name="cwriter" value="<%=cv.getCwriter()%>"></td>
					</tr>
					<tr>
						<td>첨부파일</td>
						<td><input type="file" name="cfile" size="200" value="<%=cv.getCfile()%>"></td>
					</tr>
				</table>
