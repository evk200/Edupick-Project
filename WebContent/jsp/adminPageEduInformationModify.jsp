...
<!-- title 아이콘 -->
		...
		function check(){
			if (document.frm.esubject.value == ""){
				alert("제목을 입력하세요.");
				document.frm.esubject.focus();
				return;
			}else if (document.frm.econtent.value == "") {
				alert("내용을 입력하세요.");
				document.frm.econtent.focus();
				return;
			}else if (document.frm.ewriter.value == "") {
				alert("작성자를 입력하세요.");
				document.frm.ewriter.focus();
				return;
			}
			alert("등록합니다.");
			document.frm.action="<%=request.getContextPath() %>/Admin2/adminPageEduInformationModifyAction.do";
	...
	<section>
		<article>
			<form class="pagefrmp" name="frm">
				<h2>공지사항 수정</h2>
				<input type="hidden" value="<%=ev.getEcategory()%>">
				<select name="ecategory" id="ncategory" style="width:120px;">
				...
				<table style="border-collapse:collapse">
					<tr>
						<td width=10%>제목</td>
						<td width=150%><input name="esubject" value="<%=ev.getEsubject()%>"></td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea name="econtent"><%=ev.getEcontent()%></textarea></td>
					</tr>
					<tr>
						<td>작성자</td>
						<td><input name="ewriter" value="<%=ev.getEwriter()%>"></td>
					</tr>
					<tr>
						<td>첨부파일</td>
						<td><input type="file" name="efile" size="200" value="<%=ev.getEfile()%>"></td>
					</tr>
				</table>
				<button type="button" id="eduinformationsubmit" onclick="check()">등록</button>
			</form>
		</article>
	</section>
</body>
