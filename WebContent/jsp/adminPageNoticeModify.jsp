...
<!-- title 아이콘 -->
		...
		function check(){
			...
			alert("등록합니다.");
			document.frm.action="<%=request.getContextPath() %>/Admin2/adminPageNoticeModifyAction.do";
	...
	<section>
		<article>
			<form class="pagefrmp" name="frm">
				<h2>공지사항 수정</h2>
				<input type="hidden" value="<%=nv.getNcategory()%>">
				<select name="ncategory" id="ncategory" style="width:120px;">
					<option value="use">use이용 안내</option>
					<option value="event">event이벤트</option>
				</select>
				...
				<table style="border-collapse:collapse">
					<tr>
						<td width=10%>제목</td>
						<td width=150%><input name="nsubject" value="<%=nv.getNsubject()%>"></td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea name="ncontent"><%=nv.getNcontent()%></textarea></td>
					</tr>
					<tr>
						<td>작성자</td>
						<td><input name="nwriter" value="<%=nv.getNwriter()%>"></td>
					</tr>
					<tr>
						<td>첨부파일</td>
						<td><input type="file" name="nfile" size="200" value="<%=nv.getNfile()%>"></td>
					</tr>
				</table>
