		...
		function check(){
			var iidx = document.getElementById("iidx").value;
				...
				if (document.frm.rcontent.value == ""){
					alert("내용을 입력하세요.");
					document.frm.rcontent.focus();
					return;
				}if (document.frm.rwriter.value == "") {
					alert("작성자를 입력하세요.");
					document.frm.rwriter.focus();
					return;
				}
				alert("등록합니다.");
				document.frm.action="<%=request.getContextPath() %>/Institute/InstituteReviewAction.do?iidx="+iidx;
		...
		<section>
			...
			<input type="hidden" id="iidx" value="<%=iv.getIidx()%>">
				<table>
					...
					<tr>
						<td class="c2">작성자</td>
						<td><input type="text" name="rwriter" value="<%=loginname %>"></td>
					</tr>
				</table>
