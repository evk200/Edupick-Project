			...
			<article class="nectmain">
				...
				<table border="2" width="800px" height="200px">
					...
					<tr height="30px">
						<td><%=ev.getEidx() %></td>
						<td><%=ev.getEwriter() %></td>
						<td><%=ev.getEsubject() %></td>
						<td><%=ev.getEwriteday() %></td>
						<td><%=ev.getEhit() %></td>
					</tr>
					<tr>
						<td colspan="5" height="300px"><%=ev.getEcontent() %></td>
					</tr>
					<tr>
						<td id="td1">첨부파일</td>
						<td colspan="4">
							...
							<%if (ev.getEfile() == null) { 
								...
								<a href="<%=request.getContextPath()%>/Admin2/fileDownload.do?Nfile=<%= ev.getEfile()%>">
								<b>다운로드</b>
							</a>
							<%} %>
						</td>			
					...
					<button type="button" id="noticelist" class="noticelist_2">목록</button>
					</table>
					</div>
				</div>		
				<div id="imgmaker"><%if (ev.getEfile() == null) { 
						out.print("이미지가 없거나 오류 입니다."); %>
					<%} else {%>
					<img src="<%=request.getContextPath()%>/filefolder/s-<%=ev.getEfile()%>">
					<%} %>
				</div>
			</article>	
		</section>
